...
@Directive({
  selector: '[mask-cpf-cnpj]',
  providers: [{
    provide: NG_VALUE_ACCESSOR,
    useExisting: MaskCpfCnpjDirective,
    multi: true
  }]
})
export class MaskCpfCnpjDirective implements ControlValueAccessor {

  onTouched: any;
  onChange: any;

  @Input('mask-cpf-cnpj') maskcpfcnpj: string;
  ...
  onKeyup($event: any) {
    ...
    if (!(($event.keyCode < 48 || $event.keyCode > 57) && ($event.keyCode < 96 || $event.keyCode > 105) && $event.keyCode != 46 && $event.keyCode != 8)) {
      ...
      if ($event.keyCode === 86 || $event.keyCode === 17 || $event.keyCode === 13 || $event.keyCode == 37 || $event.keyCode == 39) {
      }
      else {
